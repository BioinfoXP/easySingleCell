...
Best100 <- function(palette_num, n = NULL) {
  if (palette_num < 1 || palette_num > 100) {
    base::stop("Invalid palette number. Please choose between 1 and 100.")
  }
  ...
  chosen_palette <- palettes[[base::paste0("Scheme", palette_num)]]
...
#' @method .get_pal numeric
#' @keywords internal
.get_pal.numeric <- function(x, n = NULL, ...) {
  base::return(Best100(base::as.integer(x), n = n))
}

#' @method .get_pal character
#' @keywords internal
.get_pal.character <- function(x, n = NULL, ...) {
  if (base::grepl("^Scheme", x)) {
    num <- base::as.numeric(base::gsub("Scheme", "", x))
    base::return(Best100(num, n = n))
  }
  base::NextMethod(".get_pal")
}
